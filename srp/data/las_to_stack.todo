...
def save_stack(stack, las_fn, min_point, max_point, las_width, las_height):
    ...
    indexes = list(range(1, len(stack)+1))
...
class Worker(object):
    ...
    def doit(self, las_fn, minx, maxx, miny, maxy, OUTPUT_METERS_PER_PIXEL, GROUND_FN, SLICE_THICKNESS, ABOVE, BELOW):
        ...
        try:
            ...
            las_min_x = int(floor(old_div((min_point[0] - minx), OUTPUT_METERS_PER_PIXEL)))
            las_min_y = int(floor(old_div((min_point[1] - miny), OUTPUT_METERS_PER_PIXEL)))
            las_max_x = int(floor(old_div((max_point[0] - minx), OUTPUT_METERS_PER_PIXEL)))
            las_max_y = int(floor(old_div((max_point[1] - miny), OUTPUT_METERS_PER_PIXEL)))
            ...
            C = np.floor((old_div((X - minx ),OUTPUT_METERS_PER_PIXEL))).astype(int) - las_min_x
            R = np.floor((old_div((Y - miny ),OUTPUT_METERS_PER_PIXEL))).astype(int) - las_min_y

            # Subtract the corresponding value from the _ground_ file to get height-above-ground.
            height_above_ground = Z - ground[old_div(( R *ground_height ),las_height), old_div(( C *ground_width ),las_width)]

            # Determine how many slices above the ground each point is at.
            level = np.floor(old_div((height_above_ground ),SLICE_THICKNESS)).astype(int)
            ...
            lvls = list(range(1 - BELOW, ABOVE+1))
